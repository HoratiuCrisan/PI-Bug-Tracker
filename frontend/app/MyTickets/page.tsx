...
const MyTickets = () => {
    ...
    useEffect(()=> {
        const fetchMyTickets = async () => {
            try {
                const response = await fetch("https://localhost:7181/api/Ticket/Get-My-Tickets/" + userData.email, {
                    method: 'GET',
                    headers: {
        ...
        <div>
          <div className='flex gap-2 font-bold my-5'>
            ...
            <h1 className='text-lg'>
              / {userData.username[0].toUpperCase() + userData.username.slice(1)}'s Tickets
            </h1>
          </div>
